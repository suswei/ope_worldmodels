...
class rollout_network(Chain):

    def __init__(self, args, vision, model):

        super(rollout_network, self).__init__()
    ...
    def forward(self, input):
        ...
        cumulative_reward = self.rollout(input,(0, 0, 0, self.args, self.vision.to_cpu(), self.model.to_cpu(), None, None, False))
        return cumulative_reward

    def rollout(self, input, rollout_arg_tuple):

        generation, mutation_idx, trial, args, vision, model, gpu, max_timesteps, with_frames = rollout_arg_tuple
        ...
        while not done:
            ...
            mean_a_t = self.action(args, z_t, h_t, c_t, gpu)
    ...
    def action(self, args, z_t, h_t, c_t, gpu):
